...
def inject_bootstrap():
    print("Injecting Bootstrap context processor", bootstrap)
...
def process_statement(file_stream):
    ...
    # Data cleaning
    df['amount'] = df['amount'].replace(r'[\$,]', '', regex=True).astype(float) # needed r before '[\$,]'
...
def dashboard():
    try:
        ...
        return render_template('dashboard_CH.html', **charts)
